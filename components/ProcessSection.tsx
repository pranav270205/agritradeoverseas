...
const processCards = [
  {
    ...
    title: 'Sourcing',
    description:
      'We carefully source the finest agricultural products directly from India\'s premier farming regions through trusted supplier networks. Our rigorous selection process, quality inspections, and sourcing expertise ensure consistency, freshness, and compliance with international standards from the very origin.',
    icon: Search,
    highlighted: false,
  },
  {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {processCards.map((card, index) => (
            <AnimatedSection key={card.title} delay={index * 0.15} className="h-full">
              <motion.div
                whileHover={{ y: -10 }}
                className={`relative rounded-2xl p-8 text-center transition-all duration-500 group h-full flex flex-col items-center ${
                  card.highlighted
              ...
              >
                ...
                {/* Icon */}
                <div
                  className={`w-16 h-16 min-h-[4rem] rounded-2xl flex items-center justify-center mx-auto mb-6 mt-2 shrink-0 ${
                    card.highlighted
                      ? 'bg-white/20'
                ...
                {/* Content */}
                <h3
                  className={`text-xl font-bold mb-3 shrink-0 ${
